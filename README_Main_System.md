...
1. **環境初始化**：建立專案結構、配置 .env 與 main.py 骨架。
2. **實作數據採集器 (Collectors)**：優先實作 Polymarket 與 FRED 數據抓取。
3. **實作專業 Agent (循序漸進)**：
   - 參閱 `Spec_Agent_Fed_Watcher.md` (貨幣政策)
   - 參閱 `Spec_Agent_Data_Analyst.md` (經濟指標)
   - 參閱 `Spec_Agent_Prediction_Specialist.md` (情緒與預測市場)
   - 參閱 `Spec_Agent_Correlation_Expert.md` (資產連動)
4. **實作總結 Agent**：參閱 `Spec_Agent_Editor_In_Chief.md`。
